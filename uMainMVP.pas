...
uses
  ...
  uViaCEPClient, Vcl.Mask ,  System.UITypes,Vcl.Dialogs;
  ...
  If Trim(StringReplace(edCEP.Text,'-','', [rfReplaceAll])) <> EmptyStr then
     begin
       try
         ...
         wQuerry.Sql.Text   := 'Select * from TCONSULTASVIACEP where bdcep='+ QuotedStr(edCEP.Text);
         wQuerry.Open;
...
begin
  FDaoConection := TConexaoDAO.Create;
end;
